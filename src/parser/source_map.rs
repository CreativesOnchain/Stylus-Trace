...
impl SourceMapper {
    ...
    pub fn lookup(&self, offset: u64) -> Option<SourceLocation> {
        ...
        if let Some(frame) = frames.next().ok().flatten() {
            let function = frame.function.and_then(|f| {
                f.demangle().ok().map(|d| d.into_owned())
            });
            
            let location = frame.location;
            
            return Some(SourceLocation {
                file: location.as_ref().and_then(|l| l.file).map(|f| f.to_string()).unwrap_or_else(|| "unknown".to_string()),
