...
pub fn generate_viewer(profile: &Profile, output_path: &Path) -> Result<()> {
    ...
    html = html.replace("<link rel=\"stylesheet\" href=\"viewer.css\">", &format!("<style>{}</style>", CSS_TEMPLATE));
    html = html.replace("<script src=\"viewer.js\"></script>", &format!("<script>{}</script>", JS_TEMPLATE));
...
pub fn generate_diff_viewer(profile_a: &Profile, profile_b: &Profile, diff_report: &serde_json::Value, output_path: &Path) -> Result<()> {
